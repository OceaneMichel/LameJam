...
public class AABB : IComparable<AABB>
{
    ...
    public int CompareTo(AABB other) 
    {
...
public class GroundManager : MonoBehaviour
{
    ...
    void AddQuad(Vector2 start, Vector2 end)
    {
        ...
        vertices[0] = new Vector3(width * start.x, 0, height * start.y); 
        vertices[1] = new Vector3(width * start.x, 0, height * end.y);
        vertices[2] = new Vector3(width * end.x, 0, height * end.y); 
        vertices[3] = new Vector3(width * end.x, 0, height * start.y);
    ...
    void CreateHoleByScanXY(Vector3 pos, float radius)
    {
        ...
        while (holeIndex < mHoles.Count)
        {
            ...
            for (int nextHoleIndex = holeIndex + 1; nextHoleIndex < mHoles.Count; nextHoleIndex++)
            {
                ...
                if (curAABB.top >= nextAABB.bottom)
                {
                    count++;
    ...
    void ScanXYCreateMesh(List<ScanXYData> datas)
    {
        ...
        for (int i = 0; i < datas.Count; i++)
        {
            ...
            AddQuad(new Vector2(0, bottom), new Vector2(1, aabb.bottom)); 

            for (int k = 0; k < childrenAABB.Count; k++)
            {
                AABB item = NormalizeAABB(childrenAABB[k]);

                AddGridWithHoles(item, mHoles); 
                AddQuad(new Vector2(item.left, aabb.bottom), new Vector2(item.right, item.bottom)); 
                AddQuad(new Vector2(item.left, item.top), new Vector2(item.right, aabb.top)); 
            }
            ...
            for (int k = 0; k < childrenAABB.Count; k++)
            {
                ...
                AddQuad(new Vector2(x, aabb.bottom), new Vector2(childAABB.left / width, aabb.top)); 
            }
            AABB lastAABB = childrenAABB[childrenAABB.Count - 1];
            AddQuad(new Vector2(lastAABB.right / width, aabb.bottom), new Vector2(1, aabb.top)); 

            bottom = aabb.top;
        }
        AddQuad(new Vector2(0, bottom), new Vector2(1, 1)); 
    }
